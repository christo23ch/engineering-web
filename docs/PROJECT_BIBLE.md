...
| **F1 — MVP** (sem. 3–8) | RF-01…RF-11: Home, servicios, portfolio, sobre nosotros, contacto, legal; CMS; formularios→CRM; SEO técnico; analítica; RGPD. **Capa de interfaz ✅ construida y auditada** (biblioteca de UI *LIBRARY APPROVED* + todas las pantallas del [§13 DESIGN_SYSTEM] + Header/Footer + SEO técnico → *FRONTEND APPROVED*, con contenido honesto: plantillas fieles y fuentes tipadas vacías, sin datos inventados). **BFF ✅ construido y probado** (2026-07-14): endpoints de captación (`/api/leads`, `/api/candidatures`) con validación en frontera y *rate limiting* durable ([§17](#17-seguridad)); persistencia PostgreSQL (Lead/Candidatura/Consentimiento, [§28](#28-modelo-de-datos)) con **outbox transaccional + worker de reintentos** (ADR-010); integraciones **Brevo** (CRM DA-2 + email DA-8) y capa de lectura **Sanity** (DA-7) con *fallback* honesto — todo operativo en cuanto existan credenciales. **Integración CMS ✅ completa** (2026-07-14): Sanity Studio como *workspace* propio (`cms/` — esquemas de las 6 entidades [§28](#28-modelo-de-datos), workflow editorial DA-4 en dos niveles con *publish* bloqueado hasta aprobación, roles, preview), plantillas §13 cableadas a los *loaders* con *fallback* honesto (build idéntico sin CMS), y **webhook de publicación firmado → *deploy hook* → rebuild SSG** (ADR-001; ISR queda como optimización DA-3, no arquitectura). Runbook en `docs/CMS.md`. Las páginas siguen 100 % SSG; solo `/api/*` es *on-demand*. **Pendiente:** altas/credenciales de proveedores (ratificación [§49](#49-anexo-de-decisiones-abiertas)), cableado de los formularios aprobados → `/api`, analítica/CMP y despliegue. | Lanzamiento del Sitio. |
| **F2 — Contenido + IA mínima** (mes 3–4) | RF-12…RF-16: blog/recursos, lead magnets, empleo, **asistente IA/búsqueda semántica (mín.)**, búsqueda interna; datos estructurados completos; CWV afinados. **Interfaz del asistente IA y de recursos/empleo/lead-magnet ✅ construida** (widget). **Backend RAG ✅ construido y probado** (2026-07-14): pipeline completa sobre pgvector (ADR-004) — chunker determinista, embeddings **Voyage** (DA-10, 512 dim), indexado idempotente + recuperación con umbral de relevancia, **Claude** vía proxy BFF (ADR-005), *prompt* RAG-anclado + *citation engine* con **política estricta anti-alucinación y cita obligatoria** (§16), **presupuesto DA-6 fail-closed** (ledger durable + hard-stop: sin presupuesto ratificado, rechaza y no gasta), caché durable, *fallback* honesto, observabilidad (`ai_events`), endpoint `/api/ia/consulta`. Runbook en `docs/AI.md`. **Pendiente:** contenido real, credenciales de proveedor y **ratificación del cliente de DA-6** (presupuesto/hard-stop) — hasta entonces el asistente permanece inerte por diseño. | IA en producción; motor de contenido. |
